...
Exactly the same as LLJ_soundings_WRF_of_Obs.py just modifed to print min and max values
'''
...
crit2 = [2 ,3600, 6000, 23.326, 11.663]
                ...
                for zz in range(len(header_lines)):
                    ...
                    # find lowest height (highest pressure) with maximum wind as laid out in Oliviera et al 2018
                        if max_wind > max_wind_threshold and max_wind_height < max_search_hgt:
                            ...
                            print 'max_wind_pres', max_wind_pres
                        ...
                        if max_wind > max_wind_threshold and max_wind_height < max_search_hgt:
                            ...
                            if (max_wind - current_min) > decrease_to_min:
                                ...
                                if len(drct_mode) != 0:
                                    ...
                                    print 'WRF LLJ count', total_count_WRF
...
counts_pres, bins_pres, patches_pres = axs2[0].hist(WRF_max_wind_pres_list, bins=np.arange(650,975,5), weights=np.ones(len(WRF_max_wind_pres_list)) / len(WRF_max_wind_pres_list) * 100)
...
fig2.text(0.04,0.97, 'Station: %s'%(Station_short), fontsize=10)
fig2.text(0.68, 0.97, 'Number of Soundings: %s'%(total_count_WRF), fontsize=10)
...
#plt.savefig(outpath + 'f000000_%s_max_wind_pres_pdf_%d_smallBox.png'%(Station, crit_num))
