...
for chosen_date_time in chosen_date_time_list:
    ...
    for folder_date in sorted_folders:
        ...
        #ncfile = Dataset(path_wrf+'/wrfout_d01_2018-12-14_00_00_00')
    ...
    #mark out areas above level specifed
                ...
                if max_wind > max_wind_threshold and max_wind_height < max_search_hgt:
                    ...
                    if (max_wind - current_min) > decrease_to_min:
                        ...
                        if len(drct_mode) != 0:
                            ...
                            lon_lat = xy_to_ll(ncfile, x, y)
                            
                            lat = lon_lat[0]
                            lon = lon_lat[1]
                            
                            plt.scatter(lon, lat, s=20, color=cc, zorder=4, transform=crs.PlateCarree())
                            #plt.barbs(y,x,to_np(u_level[y,x]),to_np(v_level[y,x]), transform=crs.PlateCarree(), length=5, zorder=4, color=cc)
                            #print 'met criteria'
    ...
    ax.scatter(-64.212, -31.298, s=50, color='cyan', transform=crs.PlateCarree(), zorder=5)
    ax.scatter(-63.726, -29.906, s=50, color='cyan', transform=crs.PlateCarree(), zorder=5)
    ...
    plt.savefig('/home/disk/meso-home/crs326/Documents/Research/PNNL_WRF/WRF_MAPS/LLJ_smooth_PNNL_WRF_%sZ_Wind_%s_full-wind_grt12ms.png' %(chosen_file_name[11:24], str(level)), dpi=200)
...
#ax.set_ylim([-39,-24])
#
#plt.savefig('/home/disk/meso-home/crs326/Documents/Research/PNNL_WRF/PNNL_WRF_Wind_%d_full-wind_grt12ms.png' %(level))

# ----------------------------------------------------------------------------------------

#PSFC = wrf.getvar(nc, 'PSFC', meta=False) # in Pa
#PSFC_hPa = PSFC/100
#
##outline = plt.contour(lons, lats, PSFC_hPa, 14, levels=[900,910,920,930,940,950,960,970,980,990,1000,1010,1020,1030,1040], colors="black", transform=crs.PlateCarree())
##ax.clabel(outline, inline=1, fontsize=10, fmt='%.0f')
#plt.contourf(lons, lats, PSFC_hPa, 14, levels=[900,910,920,930,940,950,960,970,980,990,1000,1010,1020,1030,1040], extend='both', transform=crs.PlateCarree(), cmap=get_cmap("jet"))
#
## Add a color bar
#plt.colorbar(ax=ax, shrink=.6)
#
## Set the map bounds
#ax.set_xlim([-75,-50])
#ax.set_ylim([-39,-24])
...
#
#plt.savefig('/home/disk/meso-home/crs326/Documents/Research/PNNL_WRF/PNNL_WRF_PSFC.png')


# ----------------------------------------------------------------------------------------


#lats, lons = wrf.latlon_coords(PSFC)
#
#cart_proj = wrf.get_cartopy(PSFC)
#
#fig = plt.figure()
#ax = plt.axes(projection=cart_proj)
#
#plt.contour(wrf.to_np(lons), wrf.to_np(lats), wrf.to_np(PSFC), 10, colors="black",
#            transform=crs.PlateCarree())
#plt.contourf(wrf.to_np(lons), wrf.to_np(lats), wrf.to_np(PSFC), 10,
#             transform=crs.PlateCarree(),
#             cmap=get_cmap("jet"))
#
## Add a color bar
