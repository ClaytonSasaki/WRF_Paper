...
#    fig3, ax3 = plt.subplots(figsize=(2*48+4, 16))
    ...
    full_plot = np.empty((len(full_mb_p),num_days*24))
    full_plot[:] = np.nan
    ...
    for folder_date in sorted_folders:
        ...
        if folder_date.startswith('20'):
            ...
            #                    
                    ...
                    full_plot[max_plot_pres_i:,total_count_WRF] = speed
                    
                    print 'full_plot', full_plot

                    #print 'full_plot shape 1', full_plot.shape
    ...
    print 'full_plot shape 2', full_plot.shape  
    print 'full_plot shape 2', full_plot.T.shape 
    ...
    #plot v-wind
    full_wind = ax.contourf(WRF_file_time_list,np.squeeze(full_mb_p),full_plot, cmap='YlOrRd', levels=np.arange(0,50,5), extend='both', zorder=1)

    #full_wind = ax.contourf(WRF_file_time_list,np.squeeze(full_mb_p),c=full_plot, cmap='seismic_r', zorder=1)
    ...
    cbar = fig.colorbar(full_wind, ax=ax, orientation='vertical')

    cbar.set_label('full wind (kts)', fontsize=30)
    ...
    #Nov 3-6
    fig.savefig('%s/%s'%(outpath, 'WRF_' + short_station + 'full_wspd_Nov03-Nov06_time_series.png'), bbox_inches='tight', dpi=120)
