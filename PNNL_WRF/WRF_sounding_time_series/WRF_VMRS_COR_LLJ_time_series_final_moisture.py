...
#    fig3, ax3 = plt.subplots(figsize=(2*48+4, 16))
    ...
    q_plot = np.empty((len(full_mb_p),num_days*24))
    q_plot[:] = np.nan
        ...
        if folder_date.startswith('20'):
            ...
            if (folder_date_dt >= input_start_day) and (folder_date_dt <= input_end_day):
                ...
                for hourly_file in sorted_hour_files:
                    ...
                    pres = wrf.getvar(nc, 'pressure') # in hPa

                    lats = wrf.getvar(nc, 'XLAT', meta=False)
                    lons = wrf.getvar(nc, 'XLONG', meta=False)
                    
                    dwpt = wrf.getvar(nc, 'td', units='degC')
                    temp = wrf.getvar(nc, 'temp', units='degC')
            ...
            #                    
                    ...
                    q_plot[max_plot_pres_i:,total_count_WRF] = q*1000 #g/kg
                    
                    print 'q_plot', q_plot

                    #print 'q_plot shape 1', q_plot.shape
    ...
    print 'q_plot shape 2', q_plot.shape  
    print 'q_plot shape 2', q_plot.T.shape 
    ...
    #plot v-wind
    q = ax.contourf(WRF_file_time_list,np.squeeze(full_mb_p),q_plot, cmap='RdYlBu', levels=np.arange(0,20,2.5), extend='both', zorder=1)

    #q = ax.contourf(WRF_file_time_list,np.squeeze(full_mb_p),c=q_plot, cmap='seismic_r', zorder=1)

    # set axes format

    ax.set_xlim([pd.to_datetime(x_start_day, format='%Y%m%d', errors='ignore'), pd.to_datetime(x_end_day, format='%Y%m%d', errors='ignore') + datetime.timedelta(days=1)])
    ...
    cbar.set_label('q (g/kg)', fontsize=30)
    ...
    #Nov 1-16
    fig.savefig('%s/%s'%(outpath, 'WRF_' + short_station + 'q_Nov01-Nov16_time_series.png'), bbox_inches='tight', dpi=120)

    #Nov 17- Dec 2
    #fig.savefig('%s/%s'%(outpath, 'WRF_' + short_station + 'q_Nov17-Dec02_time_series.png'), bbox_inches='tight', dpi=120)

    #Dec 3-18
    #fig.savefig('%s/%s'%(outpath, 'WRF_' + short_station + 'q_Dec03-Dec18_time_series.png'), bbox_inches='tight', dpi=120)
